...
static int __init gamepad_init(void)
{
	...
	//err = alloc_chrdev_region(dev_num, 0, 1, device_name); This did not work, not sure why
	
	majorNumber = register_chrdev(0, device_name, &fops);
	*dev_num = MKDEV(majorNumber, 0);
	printk("DEBUG: driver gamepad major_num %d\n", majorNumber);
...
static int gamepad_gamepad (struct inode *inode, struct file *filp) 
{
	...
	printk("DEBUG: GPIO Driver opened! :D \n");
	if (region == NULL) {
		err = -1;
		goto return_err;
	}
    ...
    *GPIO_IEN = 0xff; 				// enable interrupts
	...
	printk("DEBUG: Gamepad Open completed \n");
	return 0;
